...
public class Grid : MonoBehaviour {
    ...
    public LayerMask TerrainMask;//This is the mask that the program will look for when trying to find obstructions to the path.
    ...
    private void Start()//Ran once the program starts
    {
        ...
        //StartCoroutine(UpdateGridProcedurally());
    }

    public void CreateGrid()
    {
        NodeArray = new Node[iGridSizeX, iGridSizeY];//Declare the array of nodes.
        Vector3 bottomLeft = transform.position - Vector3.right * vGridWorldSize.x / 2 - Vector3.up * vGridWorldSize.y / 2;//Get the real world position of the bottom left of the grid.
        for (int x = 0; x < iGridSizeX; x++)//Loop through the array of nodes.
        {
            for (int y = 0; y < iGridSizeY; y++)//Loop through the array of nodes
            {
                Vector3 worldPoint = bottomLeft + Vector3.right * (x * fNodeDiameter + fNodeRadius) + Vector3.up * (y * fNodeDiameter + fNodeRadius);//Get the world co ordinates of the bottom left of the graph
                bool Wall = true;//Make the node a wall

                //If the node is not being obstructed
                //Quick collision check against the current node and anything in the world at its position. If it is colliding with an object with a TerrainMask,
                //The if statement will return false.

                if (Physics2D.OverlapCircle(worldPoint, fNodeRadius, TerrainMask)) //worldPoint + new Vector3(fNodeRadius, fNodeRadius, 0), TerrainMask))
                {
                    Wall = false;
                }

                //if (Physics.CheckSphere(worldPoint, fNodeRadius, TerrainMask))
                //{
    ...
    public void UpdateGrid()
    {
        for (int x = 0; x < iGridSizeX; x++)//Loop through the array of nodes.
        {
            for (int y = 0; y < iGridSizeY; y++)//Loop through the array of nodes
            {
                Vector3 worldPoint = NodeArray[x, y].vPosition; //Get the world co ordinates of the bottom left of the graph
                bool Wall = true;//Make the node a wall
    ...
    public IEnumerator UpdateGridProcedurally()
    {
        while (GameManager.instance.roundInProgress)
        {
            for (int x = 0; x < iGridSizeX; x++)//Loop through the array of nodes.
            {
                for (int y = 0; y < iGridSizeY; y++)//Loop through the array of nodes
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireCube(transform.position, new Vector3(vGridWorldSize.x, vGridWorldSize.y, 1));//Draw a wire cube with the given dimensions from the Unity inspector
