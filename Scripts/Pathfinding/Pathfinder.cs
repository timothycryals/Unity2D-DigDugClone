...
public class Pathfinder : MonoBehaviour {
    ...
    {
        GridReference = GameManager.instance.grid;//Get a reference to the game manager
    }
    ...
    List<Node> GetFinalPath(Node a_StartingNode, Node a_EndNode)
    {
        ...
        //gameObject.GetComponent<EnemyRotator>().Path = FinalPath;
        return FinalPath;
    }
    ...
    {
        float x = Random.Range(GridReference.StartPosition.position.x - GridReference.vGridWorldSize.x/2, GridReference.StartPosition.position.x + GridReference.vGridWorldSize.x / 2);
        float y = Random.Range(GridReference.StartPosition.position.y - GridReference.vGridWorldSize.y/ 2, GridReference.StartPosition.position.y + GridReference.vGridWorldSize.y / 2);
        return new Vector3(x, y, 0);
    }

    public Vector3 generateRelativePoint(Vector3 position)
    {
        float x = position.x - GridReference.StartPosition.position.x;
        float y = position.y - GridReference.StartPosition.position.y;
        return new Vector3(x, y, 0);
    }
}
